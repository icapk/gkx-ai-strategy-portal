...
import { useReadingPrd } from './useReadingPrd.ts'
import { ReadingPrdWorkspace } from './ReadingPrdWorkspace.tsx'

export function ReadingReviewSidebar() {
  const {points:basePoints,numbering,parentNumber}=useReviewCatalog('reading',catalogPoints)
  ...
  const [reviewMode,setReviewMode]=useState<'design'|'prd'|'annotations'>(()=>{const value=new URLSearchParams(location.search).get('review');return value==='prd'||value==='annotations'?value:'design'})
  const prdMode=reviewMode==='prd'
  const setPrdMode=(value:boolean)=>setReviewMode(value?'prd':'design')
  ...
  useEffect(()=>{const restore=()=>{const mode=new URLSearchParams(location.search).get('review');protect(()=>{setReviewMode(mode==='prd'||mode==='annotations'?mode:'design');setFilters(f=>({...f,mode:'design'}))})};window.addEventListener('popstate',restore);return()=>window.removeEventListener('popstate',restore)})
  ...
  const annotations=useProductAnnotations('reading',sharedPrd.book,points)
  const [filters,setFilters]=useState(()=>({...initialFilters(),mode:'design' as const}))
  ...
  useEffect(()=>{const mode=reviewFocusRequest?.target?.reviewMode;if(mode&&reviewFocusRequest?.module==='reading')protect(()=>{setCollapsed(false);setReviewMode(mode);modeUrl(mode)})},[reviewFocusRequest?.sequence])
  const openAnnotation=(id:string)=>protect(()=>{setSelected(null);setNote('');annotations.setOpenId(id);setReviewMode('annotations');modeUrl('annotations')})
  const createAnnotation=(kind:'compliance'|'prd',id:string)=>protect(()=>{setSelected(null);setNote('');annotations.setCreateRequest({key:Date.now(),kind,id});setReviewMode('annotations');modeUrl('annotations')})
  const openAnnotationFeature=(kind:'compliance'|'prd',id:string)=>protect(()=>{if(kind==='prd'){openPrd(id);return}if(!data.records[id]){setMessage('该功能点已不存在');return}setReviewQuery('');setFilters({...initialFilters(),mode:'design'});setReviewMode('design');modeUrl('design');select(id)})
  useEffect(()=>{
    if(!dirty)return
  ...
  return <>
    <aside className={`reading-review ${reviewMode!=='design'?'is-prd':''} ${collapsed?'is-collapsed':''}`} aria-label={productLabels.reading+'评审'}>
      {collapsed?<button className="review-expand" title="展开评审" aria-label="展开评审" onClick={()=>setCollapsed(false)}><img src="/assets/reading/outline.svg" alt=""/><span>评审</span></button>:<>
        <header><nav className="review-product-switch" aria-label="切换产品"><button aria-pressed={false} onClick={()=>protect(()=>{const url=new URL(location.href);url.searchParams.set("view","research");url.searchParams.set("reviewOpen","1");location.assign(url.href)})}>{productLabels.research}</button><button aria-pressed={true} onClick={()=>protect(()=>{const url=new URL(location.href);url.searchParams.set("view","reading");url.searchParams.set("reviewOpen","1");location.assign(url.href)})}>{productLabels.reading}</button></nav><button title="收起侧栏" aria-label="收起评审侧栏" onClick={()=>protect(()=>setCollapsed(true))}>‹</button></header>
        <div className="review-top-tools">
          <label className="review-mode-select"><select aria-label="评审模式" value={reviewMode} onChange={e=>{const mode=e.target.value as typeof reviewMode;protect(()=>{cancelFocus();setSelected(null);setNote('');setReviewMode(mode);setReviewQuery('');modeUrl(mode)})}}><option value="design">合规审查模式</option><option value="prd">PRD模式</option><option value="annotations">注释模式</option></select></label>
          <label className="review-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg><input aria-label={reviewMode==='annotations'?'搜索注释':prdMode?'搜索PRD全文':'搜索功能编号或标题'} value={reviewMode==='annotations'?annotationQuery:reviewQuery} onChange={e=>reviewMode==='annotations'?setAnnotationQuery(e.target.value):setReviewQuery(e.target.value)} placeholder={reviewMode==='annotations'?'搜索编号、标题或正文':prdMode?'搜索需求、规则或状态':'搜索编号或标题'}/></label>
        </div>
        {editing&&points.find(p=>p.id===editing)?<ComplianceEditor onGuard={guard=>{editGuard.current=guard}} key={editing} point={points.find(p=>p.id===editing)!} record={data.records[editing]} product="reading" features={sharedPrd.book?.revisions.find(v=>v.id===sharedPrd.book?.current)?.areas.flatMap(a=>a.features)||[]} linked={data.records[editing].relationIds??(sharedPrd.book?.revisions.find(v=>v.id===sharedPrd.book?.current)?.areas.flatMap(a=>a.features).filter(f=>f.compliance?.includes(editing)).map(f=>f.id)||[])} onCancel={()=>{editGuard.current=null;setEditing(null)}} onSave={patch=>{if(!persist({version:1,records:{...data.records,[editing]:{...data.records[editing],...patch,history:[]}}}))return false;setSelected(editing);setStatus((patch.status??data.records[editing].status));setPriority(patch.priority??data.records[editing].priority);editGuard.current=null;setEditing(null);return true}}/>:reviewMode==='annotations'?<AnnotationWorkspace query={annotationQuery} product="reading" version={annotations.version} features={annotations.catalog} store={annotations.store} openId={annotations.openId} onOpened={()=>annotations.setOpenId(null)} createRequest={annotations.createRequest} onCreateHandled={()=>annotations.setCreateRequest(null)} onOpenFeature={openAnnotationFeature} onGuardChange={guard=>{annotations.leaveGuard.current=guard}}/>:prdMode?(sharedPrd.book?<ReadingPrdWorkspace annotationTools={f=><FeatureAnnotationSection kind="prd" id={f.id} items={annotations.store.items} onOpen={openAnnotation} onCreate={()=>createAnnotation('prd',f.id)}/>} book={sharedPrd.book} revisionNumber={sharedPrd.revision} save={sharedPrd.save} query={reviewQuery} clearQuery={()=>setReviewQuery('')} opened={prdOpened} onOpened={()=>setPrdOpened(null)} onCompliance={id=>{setPrdMode(false);modeUrl('design');select(id)}} onLocate={(id,label)=>{requestFocus(id,label,'reading');if(window.innerWidth<1280&&!id.includes('::READ-review'))setCollapsed(true)}}/>:<p role="status">{sharedPrd.status}</p>):point&&current?<div className="review-detail">
          ...
          <div className="review-feature-title"><h2>{numbering(point)} {point.title}</h2><ReviewActions point={point} product="reading" onLocate={()=>focusPoint(point.id)} onRename={()=>protect(()=>setEditing(point.id))}/></div>
          ...
          <section className="compliance-related"><h3>关联功能点</h3>{sharedPrd.book?.revisions.find(v=>v.id===sharedPrd.book?.current)?.areas.flatMap(a=>a.features).filter(f=>(current.relationIds?current.relationIds.includes(f.id):f.compliance?.includes(point.id))).map(f=><button className="prd-related" key={f.id} onClick={()=>openPrd(f.id)}>{f.id} · {f.title} →</button>)}{!sharedPrd.book?.revisions.find(v=>v.id===sharedPrd.book?.current)?.areas.some(a=>a.features.some(f=>(current.relationIds?current.relationIds.includes(f.id):f.compliance?.includes(point.id))))&&<p>暂无关联PRD功能。</p>}</section>
          ...
          <div className="review-results" ref={resultsRef}>{groups.map((name,i)=>{
            ...
            return list.length?<section key={name}><h3>{i+1}. {name} <small>{list.length}</small></h3>
              {list.map(p=><article className={`review-point ${lastViewed===p.id?'is-last-viewed':''}`} key={p.id}><div className="review-feature-title"><button className="review-point-open" onClick={()=>select(p.id)}><b>{markMatch(numbering(p))}</b> {markMatch(p.title)}</button><ReviewActions point={p} product="reading" onLocate={()=>focusPoint(p.id)} onRename={()=>protect(()=>setEditing(p.id))}/></div><div className="review-point-meta"><select className="review-quick-status" aria-label={numbering(p)+' 合规状态'} data-status={data.records[p.id].status} value={data.records[p.id].status} onChange={e=>quickStatus(p.id,e.target.value as Status)}>{statuses.map(s=><option data-status={s} key={s}>{s}</option>)}</select><span>{data.records[p.id].priority}</span><button className="review-point-detail" aria-label={numbering(p)+' 审核详情'} onClick={()=>select(p.id)}>详情</button></div></article>)}
            </section>:null
        ...
        <div className="review-bottom-tools"><div id="reading-review-version"/><div className="review-bottom-action-row">{reviewMode==='design'&&<><button onClick={()=>protect(exportFile)}>导出合规审查结果</button><button onClick={()=>protect(()=>fileRef.current?.click())}>导入合规审查结果</button></>}<div id="reading-review-tools"/><DemoDataControls product="reading"/></div></div>
