 ...
 for(const v of b.revisions){
  ...
  for(const a of v.areas){
   ...
   for(const f of a.features){
    ...
    if(!f||!validId(f.id)||ids.has(f.id)||typeof f.title!=='string'||!f.title.trim()||!['P0','P1','P2'].includes(f.priority)||typeof f.release!=='string'||!Array.isArray(f.compliance)||f.compliance.some(x=>!validId(x))||!Array.isArray(f.links)||f.links.some(x=>!x||!validId(x.id)||typeof x.label!=='string')||!Array.isArray(f.rules)||!f.rules.length||f.rules.some(g=>!g||typeof g.title!=='string'||!g.title.trim()||!Array.isArray(g.items)||!g.items.length||g.items.some(x=>typeof x!=='string'||!x.trim())))return false
    ids.add(f.id)
    if(f.manual){
     if(f.manual.target?.product!=='reading'||!Array.isArray(f.manual.regions)||f.manual.regions.length>20)return false
     for(const r of f.manual.regions)if(!r||typeof r.selector!=='string'||!r.selector.trim()||r.selector.length>1000||typeof r.label!=='string'||![r.x,r.y,r.width,r.height].every(Number.isFinite)||r.x<0||r.y<0||r.width<=0||r.height<=0||r.x+r.width>1.00001||r.y+r.height>1.00001)return false
    }
   }
  }
