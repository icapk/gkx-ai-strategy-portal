import { useEffect, useRef, useId, type FormEvent, type ReactNode } from 'react'
...
export function Modal({
  ...
  const reviewEdge=0,reviewTop=0
  const titleId=useId()
  ...
  return (
    <div className="modal-backdrop review-sidebar-modal" style={reviewTop ? { paddingTop: reviewTop + 8 } : reviewEdge ? { paddingLeft: reviewEdge + 20 } : undefined} role="presentation" onMouseDown={() => { if (!document.querySelector('.annotation-draw-layer,.annotation-dialog-backdrop')) onClose() }}>
      <form
        ref={dialogRef}
