...
import {initialReadingBook,type ReadingBook} from './prdBook.ts'
import {validReadingBook} from '../../server/readingPrdValidation.mjs'
import {migrateReadingBook} from './prdMigration.ts'
import {readingAreas} from './catalog.ts'
...
export function useReadingPrd(){
 ...
 return {book:snapshot.book,revision:snapshot.revision,status:(runtime.storageMode==='browser')?status.replaceAll("共享","本浏览器").replaceAll("服务","存储"):status,save}
}
