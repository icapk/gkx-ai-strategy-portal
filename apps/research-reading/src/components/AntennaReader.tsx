...
export function AntennaReader({onBack,notes,onNotesChange,onEditingNoteChange}:Props) {
  ...
  const showSelectionCard=()=>{
    ...
    if(card){setSelectionCard(card);setRight(true)}
  }
  ...
  return <div className="antenna-reader" ref={root}>
    ...
    {selectionCard&&<div className="antenna-selection-card" role="dialog" aria-label="选区翻译与短语释义"><header><strong>{selectionCard.text.length>45?'选段翻译':'选词翻译'}</strong><button aria-label="关闭选区结果" onClick={()=>setSelectionCard(null)}>×</button></header><h4>译文</h4><p>{selectionCard.translation||'暂无此选段的可靠译文。可继续保存原文摘录。'}</p>{selectionCard.definitions.length>0&&<div>{selectionCard.definitions.map(item=><article key={item.phrase}><b>{item.phrase}</b><span>{item.meaning}</span></article>)}</div>}<footer><label className="antenna-save-option"><input type="checkbox" disabled={!selectionCard.translation} checked={!!selectionCard.translation&&includeTranslation} onChange={e=>setIncludeTranslation(e.target.checked)}/>包含译文</label><button onClick={()=>{makeNote(formatNoteParts({original:selectionCard.text,translation:includeTranslation?selectionCard.translation:'',note:''}),selectionCard.anchor,selectionCard.rects,undefined,includeTranslation&&selectionCard.translation?['翻译']:[]);setSelectionCard(null)}}>保存到笔记</button></footer></div>}
