  ...
  return <section className="reading-files" aria-label="智能阅读文献库">
    ...
    <div className="reading-files-body">
      ...
      <main className="reading-files-main"><div className="reading-files-heading"><h1>{sections.find(s => s.id === section)?.title}</h1><span>{list.length} 个文件</span></div>
        <div className="reading-files-table-scroll"><table aria-label={`${sections.find(s => s.id === section)?.title}文献列表`}><thead><tr><th>名称</th><th>作者</th><th>位置</th><th>{section === 'recent' ? '最近浏览' : '类型'}</th><th>操作</th></tr></thead><tbody>{list.slice((page-1)*pageSize,page*pageSize).map(d => <tr key={d.id}>
