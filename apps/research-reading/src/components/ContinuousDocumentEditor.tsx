...
export function ContinuousDocumentEditor({ documentItem, onClose, onSave }: Props) {
  ...
  const control = (label: string, run: () => void, active = false) => <button type="button" data-focus-id={`editor-${label}`} aria-pressed={active} onMouseDown={(event) => event.preventDefault()} onClick={run}>{label}</button>
  return <section className="continuous-editor" aria-label={`文档编辑：${title}`}>
    <header><button type="button" onClick={() => { if (!dirty || window.confirm('有未保存修改，确定离开？')) onClose() }}>返回</button><input aria-label="文档标题" value={title} maxLength={50} onChange={(event) => { setTitle(event.target.value); setDirty(true) }} /><span role="status">{dirty ? '未保存' : '已保存'}</span><button type="button" onClick={save}>保存</button></header>
    <div className="continuous-toolbar" role="toolbar" aria-label="文档编辑工具">
      {control('撤销', () => editor.chain().focus().undo().run())}{control('重做', () => editor.chain().focus().redo().run())}
      ...
      {control('无序列表', () => editor.chain().focus().toggleBulletList().run())}{control('有序列表', () => editor.chain().focus().toggleOrderedList().run())}{control('引用', () => editor.chain().focus().toggleBlockquote().run())}
      {control('表格', () => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run())}
      ...
      {control('网页书签', () => { const url = window.prompt('输入网页地址（https://）'); if (!url) return; try { const parsed = new URL(url); if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error(); const label = window.prompt('书签名称', url) || url; editor.chain().focus().insertContent(`<a href="${escape(parsed.href)}">${escape(label)}</a> `).run() } catch { setMessage('请输入有效的 http 或 https 网页地址') } })}
    ...
    <div className="continuous-paper-scroll"><EditorContent editor={editor} className="continuous-paper" /></div>
