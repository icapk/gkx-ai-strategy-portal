import {validAnnotation,parseAnnotationFile} from './annotations/importExport'
import {validReadingBook} from '../server/readingPrdValidation.mjs'
import type {Product} from './annotations/model'
...
export async function loadReviewSeed(product:Product,kind:'prd'|'annotations'){
 const response=await globalThis.fetch(import.meta.env.BASE_URL+`demo/${product}-${kind}.json`,{cache:'no-store'})
 if(!response.ok)throw Error('无法读取最新评审示例，请稍后重试')
 const value=await response.json();if(!validReview(value,product,kind))throw Error('评审示例格式无效，未修改已保存内容');return value
}
export async function withReviewLock<T>(product:Product,work:()=>Promise<T>):Promise<T>{
 ...
 return navigator.locks.request('gkx-review:'+product,work)
}
export async function reviewRequest(endpoint:string,init?:RequestInit):Promise<Response>{
 if(!import.meta.env.PROD)return globalThis.fetch(endpoint,init)
