...
import { annotationUpdateTime, annotationUpdateDay, compareAnnotations } from './presentation'
import {RangeCorrectionEditor} from '../prototypeFocus/RangeCorrectionEditor'
import {facetCount} from '../facetCounts'
import { useEffect, useLayoutEffect, useRef, useState } from 'react'
import { ReviewBottomTools } from '../researchReview/PrdPresentation'
import { captureAnnotationRestore } from './restoration'
import { annotationSources, annotationStatuses, shanghaiDate, type Annotation, type AnnotationInput } from './model'
import { parseAnnotationFile } from './importExport'
import { AnnotationComposer } from './AnnotationComposer'
import { AnnotationDrawing, AnnotationOverlay } from './AnnotationOverlay'
import { captureAnnotationRange, currentAnnotationContext, rangeMatchesContext, rangeFrames, savedRangeMatchesContext } from './location'
import { usePrototypeFocus } from '../prototypeFocus/FocusContext'
import type { AnnotationRange, AnnotationWorkspaceProps } from './uiTypes'
...
export function AnnotationWorkspace({ product, version, features, store, openId, onOpened, createRequest, onCreateHandled, onOpenFeature, pageContext, currentTarget, onGuardChange, query = '' }: AnnotationWorkspaceProps) {
  ...
  return <div className="annotation-workspace" ref={workspaceRef} onScroll={e=>{if(!selected&&!showRemoved)listScroll.current=e.currentTarget.scrollTop}}>
    ...
    <p className="annotation-hint">{store.ready ? (import.meta.env.PROD?'注释仅保存在当前浏览器，不影响其他访客。':'注释由本地服务统一保存，浏览器间自动同步。') : '正在连接并迁移注释，请稍候…'}{annotating && ' Esc取消当前动作'}</p>
