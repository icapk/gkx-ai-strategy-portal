import { prdRequest, prdModeLabel } from '../prdTransport'
...
export function useReadingPrd(){
 ...
 const save=async(book:ReadingBook,expectedRevision=latest.current.revision)=>{
  ...
  busy.current=true
  try{
   const res=await prdRequest(endpoint,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({expectedRevision,book})})
   const next=await res.json()
   if(res.status===409){accept(next);setStatus('保存冲突：共享记录已更新，当前输入仍保留。');return false}
   if(!res.ok)throw Error(next.error||'共享保存失败')
   accept(next);setStatus(`已保存 · ${prdModeLabel} · 修订 ${next.revision}`);return true
 ...
 useEffect(()=>{
  ...
  const pull=async()=>{if(running||busy.current)return;running=true;try{
   const res=await prdRequest(endpoint,{cache:'no-store'});const next=await res.json();if(!res.ok)throw Error(next.error||'读取失败')
   if(busy.current)return
   accept(next)
   if(!next.book){await save(initialReadingBook(),0)}else {const upgraded=migrateReadingBook(next.book,readingAreas);if(upgraded!==next.book)await save(upgraded,next.revision);else setStatus(`${prdModeLabel} · 修订 ${latest.current.revision}`)}
