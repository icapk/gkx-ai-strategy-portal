...
export interface ReviewRecord { status: Status; priority: Priority; history: ReviewEvent[] }
...
export function parseReview(value: unknown): ReviewFile {
  ...
  return {version:1,records:JSON.parse(JSON.stringify(file.records))}
}
export function loadReview(): {data:ReviewFile; error:string} {
  try {const raw=localStorage.getItem(STORAGE_KEY); const data=freshReview(); if(!raw)return {data,error:''}; const saved=parseReview(JSON.parse(raw)); const records={...data.records}; for(const [id,record] of Object.entries(saved.records)) records[id]=record; return { data: {version:1, records}, error:'' }}
  catch {return {data:freshReview(), error:'本地审核记录无法读取，暂显示AI初审基线。'}}
}
export interface Filters { mode:'design'|'story'|'prd'; group:string; parent:string; child:string; stories:number[]; statuses:Status[]; priorities:Priority[] }
