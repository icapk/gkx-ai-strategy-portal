...
- 行内操作：表单设计器、表单打印、删除；点击行也可进入设计工作区。
- 设计工作区顶部只保留一处可修改的表单名称；不在设计器内部重复显示第二个表单名称。
- 顶部操作：预览、表单打印、保存表单；设计工作区显性标注表单规则。
...
- 左侧为组织管理树，支持无限层级展开/收起和组织检索；标题行显性标注组织数据管理、组织列表查看，节点“...”菜单包含新增子组织、重命名、组织数据管理。
