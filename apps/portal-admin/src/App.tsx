          ...
          <div className="business-directory-node" key={node.id}>
            <div className="business-directory-row" style={{ paddingLeft: `${8 + level * 20}px` }}>
              ...
              <button
                ...
                className={`business-directory-item ${selectedId === node.id ? "active" : ""} ${node.disabled ? "is-disabled" : ""}`.trim()}
                onClick={() => onSelect(node.id)}
              >
                <FolderOpen size={16} /><span>{node.name}</span><b>{node.disabled ? "已禁用" : node.count}</b>
              </button>
            </div>
    ...
    <div className="business-resource-layout">
      <aside className="business-directory">
        ...
        <div className="business-directory-actions">
          ...
          {type === "talent" && <button type="button" disabled={!selectedDirectory} onClick={() => {
            if (!selectedDirectory) return;
            setDirectories((list) => updateBusinessDirectory(list, selectedDirectory.id, (node) => ({ ...node, disabled: !node.disabled })));
            notify(selectedDirectoryDisabled ? "人才库已启用" : "人才库已禁用", "success");
          }}>{selectedDirectoryDisabled ? "启用" : "禁用"}</button>}
